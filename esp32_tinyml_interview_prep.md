...
A: Yes, that's a real risk with ReLU — if a bottleneck neuron's pre-activation goes negative for all inputs during training, it dies and stays at zero permanently ("dying ReLU"), effectively throwing away one of your two latent dimensions. I addressed this directly: the bottleneck layer now uses Leaky ReLU (`alpha=0.01`) instead of plain ReLU, so a neuron with a negative pre-activation still has a small gradient and can recover rather than dying outright. I also switched every ReLU-family layer to He-normal initialization (tuned for ReLU-style activations, versus Keras's Glorot default which is tuned for tanh/sigmoid) and added gradient clipping (`clipnorm=1.0`) to the optimizer, so a single large early update can't push a neuron permanently negative. I didn't just assume this fixed it, either — I added a diagnostic right after training that runs the validation set through the latent layer and checks each of the 2 dimensions for `max|activation| < 1e-6`, flagging it as dead if so, so I have an actual verification step rather than a hope.

**Q7b. Did that change ripple anywhere else?**
A: Yes — the C++ forward pass in `inference.cpp` has to match Keras's activation exactly, or the deployed model's output won't match what was trained. I replaced the old `bool use_relu` toggle in `dense()` with an `Activation` enum (`RELU` / `LEAKY_RELU` / `SIGMOID`) and added a `leaky_relu(x) = x > 0 ? x : 0.01f*x` function matching Keras's `alpha=0.01`, then switched specifically the bottleneck's `dense()` call (`W_enc2`/`B_enc2`, producing `latent`) to use it — the other three layers (`enc1`, `dec1`, `dec2`) are unchanged. One important detail: the array *shapes* in `model_weights.h` didn't change, so the old header still compiles fine, but the actual weight *values* were fit against the old architecture — `train.py` needs to be re-run to regenerate `model_weights.h` before this takes effect on real hardware. If asked "is this live on the device right now," the honest answer is: the code path is there, but it needs a retrain-and-reflash cycle to actually matter.
