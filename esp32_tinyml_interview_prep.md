...
A: Sample rate is 5Hz (`SAMPLE_INTERVAL=200ms`), and inference itself is 19µs — so inference consumes roughly 0.01% of each sample period. The rest of the 200ms is mostly idle CPU time in the original blocking design (spent inside `pulseIn()` waiting for the echo, or now, in the non-blocking version, available for other work) — HC-SR04's own physical constraint is you shouldn't trigger faster than about every 60ms anyway, since a stale echo from the previous pulse can bleed into the next reading.
...
## The ISR / non-blocking gap — full honest answer

If asked to open `sensor.cpp` live: the version I originally wrote uses `pulseIn()`, which blocks the CPU for up to the timeout duration while waiting for the echo. I've since worked through and understand the fix — an ISR (`attachInterrupt` on `CHANGE`) that timestamps the rising and falling edges of the ECHO pin asynchronously, paired with a small state machine (`IDLE → WAITING → READY/TIMED_OUT`) driven from `poll()`, so `loop()` never stalls. I'd present this as "here's the limitation I identified in my own design, and here's the fix" rather than claiming the original version was already non-blocking — that's the honest and, frankly, more impressive answer than pretending there was never a gap.
